...
public class Kamikazi_enemy : MonoBehaviour
{
    ...
    public float dropEnergy = 20;

    public float dropMoney = 20;
    ...
    private void OnTriggerEnter(Collider player)
    {
        ...
        if (stats = player.GetComponent<playerHP>())
        {
            stats.ChangeHealth(-15);
        }

        shieldhpscript status;

        if (status = player.GetComponent<shieldhpscript>())
        {
            status.ChangeHealth(-15);
        }
